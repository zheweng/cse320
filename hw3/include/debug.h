...
#else
#define debug(S, ...)
#define error(S, ...)   do{fprintf(stderr, KRED "ERROR: " KNRM S,   __VA_ARGS__  );}while(0)
#define warn(S, ...)    do{fprintf(stderr, KYEL "WARN: " KNRM S,    __VA_ARGS__  );}while(0)
#define info(S, ...)    do{fprintf(stderr, KBLU "INFO: " KNRM S,    __VA_ARGS__  );}while(0)
#define success(S, ...) do{fprintf(stderr, KGRN "SUCCESS: " KNRM S, __VA_ARGS__  );}while(0)
#endif
